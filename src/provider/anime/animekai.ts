...
class Animekai extends BaseClass {
  ...
  constructor(options: ClientConfig = {}, baseUrl: string = 'https://anikai.to') {
    super(options);
    this.baseUrl = baseUrl;

    this.megaup = new MegaUp(options);
    this.headers = {
  ...
  async search(query: string, page: number = 1): Promise<IAKPaginated<IAKAnime[] | []>> {
    ...
    try {
      const response = await this.client.get(`${this.baseUrl}/browser?keyword`, {
        params: {
   ...
   * @param  genre - The genre which to fetch anime (required)
   * @param {number} page - Page number for pagination (default: 1)
  ...
  private parseEpisodes($: cheerio.CheerioAPI, animeId: string) {
    ...
    $(selector).each((_, element) => {
      ...
      let Sub;
      if (language) {
        Dub = Number(language) >= 3;
        Sub = Number(language) >= 1;
      }
      episodes.push({
