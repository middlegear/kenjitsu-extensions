...
import {
  ...
  type IZSpotlight,
  type AniwatchServers,
} from '../../types/anime/zoro.js';

/**
 * A class for interacting with the Aniwatch platform (aniwatchtv.to) to search for anime, fetch detailed information,
...
export class Aniwatch extends BaseClass {
  ...
  constructor(baseUrl: string = 'https://aniwatchtv.to', options: ClientConfig = {}) {
    super(options);
    ...
    $(selector).each((_, element) => {
      anime.push({
        id:
          ...
          $(element).find('a.film-poster-ahref').attr('href')?.split('/').at(1)?.split('?').at(0) ||
          null,
  ...
  private parseSearchSuggestions($: cheerio.CheerioAPI): IResponse<IZSearchSuggestions[] | []> {
    ...
    $('a.nav-item').each((_, element) => {
      ...
      animeSuggestions.push({
        id: $(element).attr('href')?.split('/').at(1)?.replace('?ref=search', '').trim() || null,
  ...
  private parseAnimeInfo($: cheerio.CheerioAPI): IZoroAnimeInfoScrape<IZAnimeInfo | null> {
    ...
    const animeInfo: IZAnimeInfo = {
      ...
      // duration: $('span.item').last().text().trim(),
    };
   ...
   */
  private parseHome($: cheerio.CheerioAPI): IZHomeResponse<IZAnime[] | []> {
  ...
  private parseServerData($: cheerio.CheerioAPI): IResponse<IServerInfo | null> {
    ...
    const filterTCloud = (serverArray: ISubServers[]): ISubServers[] => {
      return serverArray.filter(server => {
        return server.serverName && !server.serverName.includes('t-cloud');
      });
    };

    servers.sub = filterTCloud(servers.sub);
    servers.dub = filterTCloud(servers.dub);
    servers.raw = filterTCloud(servers.raw);
   ...
   */
  private findServerId(servers: IServerInfo, category: ISubOrDub, server: AniwatchServers): number {
    const serverPreference: AniwatchServers[] = ['megacloud', 'vidsrc'];
    ...
    let usedServer: AniwatchServers | null = null;
   ...
   * @param {string} episodeId - The unique identifier for the episode (required).
   * @param {AniwatchServers}  server - The streaming server to use (optional, defaults to megacloud). Note: vidsrc may return a 403 error due to CORS restrictions; use a proxy or switch to megacloud
  ...
  async fetchSources(
    episodeId: string,
    server: AniwatchServers = 'megacloud',
    ...
    try {
      const fetchedServers = await this.fetchServers(episodeId);
      if (fetchedServers.error) {
        return {
          data: null,
          headers: { Referer: null },
      ...
      const serverId = this.findServerId(fetchedServers.data as IServerInfo, category, server);
