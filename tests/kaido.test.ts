...
import { Kaido } from '../src/provider/anime/kaido.js';
...
test('returns an array based on search suggestions query', async () => {
  const data = await zoro.searchSuggestions('sentenced');
  expect(Array.isArray(data.data)).toBe(true);
  expect(data.data.length).toBeGreaterThan(0);
});
...
test('returns an object containing streaming sources', async () => {
  const data = await zoro.fetchSources('solo-leveling-18718-episode-119497', 'vidcloud', 'sub');
  expect(data.data).not.toBeNull();
